...
## 15. 第三次证伪：架构实验发现的 live/replay 分歧与共享引擎修复（2026-08-27）

回放此前是规则的第二份手写实现。一次"跳出当前架构"的重设计实验
（worktree 分支 `redesign-v5`）把实盘与回放捏进同一份纯判断函数时，
发现两者语义**已经**分叉——散文架构下没有任何机制能发现这类分歧：

| # | 分歧 | 后果 | 修复 |
|---|---|---|---|
| 1 | drift 告警触发后指纹永久记住 `over`；滞回重武装后再次出带，状态串相同 → novelty gate 永久压制。drawdown 二轮 episode 进同档位同理。回放没模拟指纹 | 页面回放承诺的告警（重复出带/二轮回撤）实盘永远不会发——**漏报**且证伪仪器失真 | 重武装/恢复时对指纹做静默状态迁移（24h 震荡冷却保留），回归测试 A4d/A4e |
| 2 | 实盘只在盘中判 β/残差，收盘 run 不判；回放按收盘残差统计 | 口径分叉：resid days 只存在于回放 | 收盘 run 加入残差判断（收盘本是权威判断点），测试 A6 |
| 3 | 实盘跳空判断无除权守卫（回放只跑收盘所以从未暴露） | 除权日早晨实盘会发"开盘跳空 −33%"假警报 | 超限价跳空归入 corporate_action，测试 A7 |

**修复形态 = 把 v5 实验的核心机制移植回本仓库**（`demo-ashare/`）：
`judgment.js`（纯判断模块，忠实提取自部署版 producer + 上述三修复）被
实盘 producer、回放、离线测试**三方共享**；`build.sh` 拼接为单文件部署物
（平台单文件模式已验证，多文件 require 不引入未知）。离线测试 26/26
（`node demo-ashare/test/run-tests.js`，约 1 秒，含全部回归点）。

**线上重部署与验证**（证据 15–17）：备份逐字节等于冻结证据 01 → 覆盖
`src/index.js` → 两次 `alva run`（NAV 连续、KV 指纹延续、novelty gate 安静、
schema 不变）→ 共享引擎回放重跑真实 12 个月：**44 告警日（3.7/月，82% 安静）
vs 旧独立回放的 49**——差的 5 天正是旧回放承诺、实盘 novelty gate 会压掉的；
两个除权日识别不变，−20% 回撤 episode 存活。页面回放面板读 feed 实时数据，
已自动显示新口径（回放 summary 为 append-only，新旧两行并存可审计）。

方法论收获：回放作为证伪仪器，自身也需要被证伪——唯一可靠的办法是让它
与被测规则共享同一份代码。crypto demo 的 producer（v2 时代设计）存在同型
指纹问题（drawdown 无 episode 状态），因其页面不承载回放承诺、且属早期
冻结演示，记为已知盲区不改（见 §14）。
