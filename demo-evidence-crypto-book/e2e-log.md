...
| 11:2x | **v1.0.3 发布（用户反馈：对齐 showcase v2.0.7"术语同位语"）**：4 处修正（HTML 2 + README 2）——`（这个指标叫"有效押注数"，算法 1/Σw²，满分 3）` → `（有效押注数 1/Σw²，满分 3）`；`（术语叫 β 和残差；对照组取其余两币平均…）` → `一起动的部分（β），和它自己独有的部分（残差）；对照组…`（解释从句移出括号成正文）。"术语叫/这个指标叫"腔调是给新手上课，读者是投资者——规则已升级于 playbook-ui.md §3。grep 自查零命中；lint 0/0/0；线上逐字节一致；逻辑无变化 | 10–13 |
